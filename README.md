...
- `systemd/agent-node@.service` - systemd: node 多实例服务文件（旧名 proxy）
...
- `CODEX_ALLOWED_PROXIES`: 可选，允许注册的 node id 列表（逗号分隔，例如 `cloud_huang_1,cloud_huang_2,void_1`）
- `CODEX_ENFORCE_PROXY_ALLOWLIST`: 可选，设为 `1` 时强制执行 allowlist（不在列表的 node 会被拒绝注册）
